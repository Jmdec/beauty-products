...
export function ProductCard({ product }: ProductCardProps) {
  ...
  return (
    <Link href={`/products/${product.id}`} className="group block">
      <article className="relative flex flex-col h-full overflow-hidden rounded-2xl bg-card border border-border/60 hover:border-primary/30 hover:shadow-lg hover:shadow-primary/5 transition-all duration-400">
        {/* ── Image ── */}
        ...
        <div className="flex flex-col flex-1 p-5 gap-3">
          ...
          <div className="flex items-center justify-between pt-3 mt-auto border-t border-border/50">
            ...
            <span className="hidden sm:flex items-center text-xs text-foreground/30 group-hover:text-primary group-hover:gap-1.5 gap-1 transition-all duration-200">
              View
